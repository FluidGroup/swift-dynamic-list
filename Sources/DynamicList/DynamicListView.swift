...
@available(iOS 13, *)
public final class DynamicListView<
  Section: Hashable & Sendable,
  Data: Hashable & Sendable
>: UIView,
   UICollectionViewDelegate, UIScrollViewDelegate
{
  ...
  public struct CellProviderContext {
    ...
    public func cell<Configuration: UIContentConfiguration>(
      ...
      if _collectionView.cellForIdentifiers.contains(_reuseIdentifier)
          == false
      {
      ...
      return self.cell(
        ...
        withConfiguration: { cell, state, customState in
          UIHostingConfiguration {
            content(state, customState).environment(\.versatileCell, cell)
          }.margins(
            .all,
            0
          )
  ...
  public init(
    ...
    self._collectionView = CollectionView.init(
      frame: .null,
    ...
    self._collectionView.contentInsetAdjustmentBehavior =
  ...
  public func state<Key: CustomStateKey>(for data: Data, key: Key.Type) -> Key
    .Value?
  {
  ...
  func _setState(cellState: CellState, for data: Data) {
    ...
    guard
      let cell = _collectionView.cellForItem(at: indexPath) as? VersatileCell
    else {
      return
    }
  ...
  public func supplementaryViewHandler(
    _ handler:
    @escaping @MainActor (SupplementaryViewProviderContext) ->
    ...
    dataSource.supplementaryViewProvider = {
      ...
      let context = SupplementaryViewProviderContext(
        _collectionView: collectionView as! CollectionView,
        indexPath: indexPath,
        kind: kind
      )
